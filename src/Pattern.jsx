...
function Pattern({ goBack }) {
  ...
  const getPredefinedLists = useCallback(() => {
    return [
      ...
      { id: '  LUCKY 48  NUMBER', name: ' LUCKY 48  NUMBER', numbers: [  ], description: 'Numbers that spell BINGO' },
      { id: '  LUCKY 48  NUMBER', name: ' LUCKY 48  NUMBER', numbers: [  ], description: 'Numbers that spell BINGO' },
      { id: '  LUCKY 48  NUMBER', name: ' LUCKY 48  NUMBER', numbers: [  ], description: 'Numbers that spell BINGO' },
     ...
     { id: '  LUCKY 48  NUMBER', name: ' LUCKY 48  NUMBER', numbers: [  ], description: 'Numbers that spell BINGO' },
       ...
       { id: 'BIMMBUY  LUCKY 35 NUMBER', name: 'BIMMBUY  LUCKY 35 NUMBER', numbers: [ 1, 2, 6, 10, 11, 12, 13, 14, 18, 19, 24, 27, 28, 30, 34, 38, 39, 40, 41, 42, 43, 44, 47, 48, 50, 56, 58, 60, 65, 68, 69, 70, 73, 74, 75], description: 'Numbers that spell BINGO' },
    
      { id: 'NITOY LUCKY 35 NUMBER', name: 'NITOY  LUCKY 35 NUMBER', numbers: [ 2, 4, 6, 7, 9, 11, 12, 14, 15, 16, 17, 18, 22, 23, 25, 28, 29, 30, 32, 33, 35, 38, 41, 45, 46, 51, 53, 56, 58, 59, 61, 62, 63, 64, 71, 73, 75,], description: 'Numbers that spell BINGO' },


     
      { id: 'CRISS', name: 'CRISS WINNING NUMBERS', numbers: [3, 5, 4, 8, 10, 11, 15, 12, 18, 20, 21, 23, 24, 25, 28, 29, 31, 34, 36, 37, 38, 39, 44, 45, 47, 49, 50, 51, 52, 53, 54, 56, 59, 60, 61, 62, 63, 65, 68, 69, 71, 72, 75], description: 'Commonly chosen numbers' },
     
      { id: 'BIMBY', name: 'BIMBY WINNING NUMBERS', numbers: [1, 3, 4, 6, 7, 8, 10, 11, 12, 15, 17, 18, 21, 22, 23, 24, 25, 26, 27, 28, 30, 31, 33, 34, 38, 39, 42, 43, 45, 45, 47, 49, 50, 51, 52, 53, 58, 59, 63, 64, 65, 66, 69, 70, 71, 72, 73], description: 'Commonly chosen numbers' },
     
      { id: 'CRISS LUCKY 4X NUMBERS', name: 'CRISS LUCKY 4X NUMBERS', numbers: [3, 5, 24, 28, 29, 47, 52, 53, 54, 59, 63], description: 'Commonly chosen numbers' },
      
      { id: 'CRISS LUCKY 6X 5X 4X NUMBERS', name: 'CRISS LUCKY 6X 5X 4X NUMBERS', numbers: [3, 4, 5, 8, 10, 11, 12, 13, 15, 17, 20, 21, 23, 24, 25, 28, 29, 31, 34, 36, 37, 38, 39, 44, 45, 47, 49, 50, 51, 53, 54, 55, 56, 57, 59, 60, 65, 67, 68, 71, 72, 73, 74, 75], description: 'Numbers ending with 7' },
      
      { id: 'CRISS AND BIMBY WINNING NUMBERS', name: 'CRISS AND BIMBY WINNING NUMBERS', numbers: [1, 3, 4, 5, 6, 8, 10, 11, 12, 12, 15, 17, 18, 21, 23, 24, 25, 26, 28, 29, 30, 31, 33, 34, 37, 38, 39, 43, 44, 45, 46, 47, 49, 50, 51, 52, 53, 54, 56, 57, 59, 63, 64, 65, 66, 67, 68, 69, 71, 72, 73, 74, 75], description: 'Round numbers by decade' },
     
      { id: 'NITOY LUCKY NUMBER', name: 'NITOY LUCKY NUMBER', numbers: [1, 3, 4, 7, 8, 9, 10, 15, 16, 17, 20, 21, 22, 23, 24, 26, 29, 31, 33, 35, 36, 38, 39, 44, 42, 50, 51, 52, 55, 56, 59, 60, 61, 62, 63, 65, 66, 67, 68, 69, 70, 73, 74], description: 'Numbers that spell BINGO' }
    ];
  }, []);
  ...
  const styles = {
    ...
    roundFrequencyButton: { padding: "0.5rem 1rem", background: "#9C27B0", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "1rem", transition: "all 0.3s ease" },
    roundHistoryButton: { padding: "0.5rem 1rem", background: "#FF9800", color: "white", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "1rem", transition: "all 0.3s ease", position: "relative" },
    ...
    roundFrequencyItem: (count, isSelected, isFavorite) => ({ aspectRatio: "1", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: isSelected ? "#ffeb3b" : count > 0 ? "#e8f4fd" : "white", border: isFavorite ? "2px solid #ffd700" : count > 0 ? "2px solid #667eea" : "2px solid #e0e0e0", borderRadius: "8px", cursor: "pointer", transition: "all 0.2s ease", fontSize: "0.8rem", fontWeight: count > 0 ? "bold" : "normal", color: count > 0 ? "#333" : "#999", position: "relative" }),
    ...
    ball: (active) => ({ aspectRatio: "1", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: active ? "#ffeb3b" : "white", border: active ? "2px solid #fbc02d" : "2px solid #e0e0e0", borderRadius: "50%", cursor: "pointer", transition: "all 0.3s ease", fontSize: "0.8rem", padding: "0.25rem", position: "relative" }),
    ...
    bingoCard: (isWinner, isPinned, hasFavorites) => ({ background: "white", border: isWinner ? "4px solid #ff4757" : isPinned ? "4px solid #ffd700" : "2px solid #333", borderRadius: "12px", padding: "1.5rem 1rem", transition: "all 0.3s ease", width: "100%", height: "100%", boxSizing: "border-box", boxShadow: "0 4px 8px rgba(0,0,0,0.1)" }),
    cardLabel: { display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.75rem", padding: "0.5rem", background: "linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)", borderRadius: "8px", fontSize: "0.9rem", cursor: "pointer", transition: "all 0.2s ease" },
    ...
    bingoCell: (isHighlighted, isFree, isFavorite, hasError) => ({ width: "45px", height: "45px", display: "flex", alignItems: "center", justifyContent: "center", border: hasError ? "3px solid #ff4757" : (isFavorite ? "2px solid #ffd700" : "1px solid #333"), borderRadius: "4px", background: isHighlighted ? "#ffeb3b" : isFree ? "#f0f0f0" : "white", fontWeight: "bold", fontSize: isFree ? "1.2rem" : "1rem", cursor: "default", position: "relative", color: "#333" }),
    ...
    emojiOption: { fontSize: "1.5rem", padding: "0.5rem", textAlign: "center", cursor: "pointer", borderRadius: "8px", transition: "all 0.2s ease" },
  ...
  useEffect(() => {
    ...
    styleElement.textContent = `
      ...
      .emoji-option:hover { background: #667eea; color: white; transform: scale(1.1); }
      .round-frequency-item:hover { transform: scale(1.1); box-shadow: 0 2px 8px rgba(0,0,0,0.2); z-index: 10; }
      ...
      <main style={styles.main}>
        ...
        <div style={styles.controlsGrid}>
          ...
          <div style={styles.controlCard}>
            ...
            <button onClick={handleNewRound} style={styles.newRoundButton} className="new-round-button">🆕 Start New Round</button>
            <button onClick={recordBingoResult} style={styles.recordResultButton} className="record-result-button" disabled={!myCards.length && !generatedCards.length || winners[currentPattern]?.length === 0}>🏆 Record BINGO Result</button>
        ...
        {showRoundHistory && (
          <div style={styles.roundHistorySection}>
            ...
            {roundHistory.length > 0 ? (
                ...
                {selectedRound && roundStatsByRound[selectedRound] && (
                  <div style={styles.roundDetail}>
                    ...
                    <div style={styles.roundStatsGrid}>
                      ...
                      <div style={styles.roundStatCard}><div style={styles.roundStatValue}>{roundStatsByRound[selectedRound].leastFrequent?.number || '-'}</div><div style={styles.roundStatLabel}>Least Frequent ({roundStatsByRound[selectedRound].leastFrequent?.count || 0}x)</div></div>
                    </div>
                    <h5 style={{ margin: "1rem 0 0.5rem 0" }}>Called Numbers (with frequency)</h5>
                    <div style={styles.roundNumbersGrid}>
                      {Array.from({ length: 75 }, (_, i) => i + 1).map(num => {
                        ...
                        if (isCalled) {
                          return (<div key={num} style={{ ...styles.roundNumberItem(isFavorite), display: "flex", flexDirection: "column", padding: "0.25rem" }} title={`Number ${num} called ${frequency} time(s)`}>
                            <span>{num}</span>
                          ...
                          return (<div key={num} style={styles.roundNumberEmpty} title={`Number ${num} not called`} />);
                        }
                      })}
                    </div>
                ...
                {roundComparison && (
                  <div style={styles.comparisonSection}>
                    ...
                    <div style={styles.comparisonGrid}>
                      ...
                      <div style={styles.comparisonItem}><div style={{ fontSize: "0.8rem", opacity: 0.9 }}>Most Unique</div><div style={{ fontSize: "1.2rem", fontWeight: "bold" }}>Round {roundComparison.mostUniqueRound.roundNumber}</div><div style={{ fontSize: "0.9rem" }}>({roundComparison.mostUniqueRound.stats.uniqueNumbers} unique)</div></div>
                    </div>
                    <div style={{ marginTop: "1rem" }}>
                      <h5 style={{ margin: "0 0 0.5rem 0", color: "white" }}>Most Frequent Numbers Overall</h5>
                      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                        {roundComparison.mostFrequentNumbers.map(({ number, count }) => (
                          <div key={number} style={{ background: "rgba(255,255,255,0.3)", padding: "0.5rem 1rem", borderRadius: "20px", fontWeight: "bold" }}>#{number} ({count}x)</div>
                        ))}
                      </div>
                    </div>
                    <div style={{ marginTop: "1rem" }}>
                      <h5 style={{ margin: "0 0 0.5rem 0", color: "white" }}>Average Column Distribution</h5>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "0.5rem" }}>
                        {Object.entries(roundComparison.columnAverages).map(([col, avg]) => (
                          <div key={col} style={{ background: "rgba(255,255,255,0.2)", padding: "0.5rem", borderRadius: "8px", textAlign: "center" }}>
                            <div style={{ fontWeight: "bold", color: getColumnColor(col) }}>{col}</div>
                            <div>{avg} avg/round</div>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <p style={{ textAlign: "center", color: "#999", padding: "2rem" }}>No rounds completed yet. Start drawing balls and complete rounds to see history!</p>
            )}
          </div>
        )}
            ...
            <div style={styles.roundFrequencyHeader}>
              ...
              {roundFrequency.length > 0 && (
                <div style={styles.roundStats}>
                  <span style={styles.roundStat}>Total Rounds: <span style={styles.roundStatValue}>{roundStats.totalRounds}</span></span>
                  <span style={styles.roundStat}>Unique Numbers: <span style={styles.roundStatValue}>{roundStats.uniqueNumbers}</span></span>
                  <span style={styles.roundStat}>Most Frequent: <span style={styles.roundStatValue}>{roundStats.mostFrequentNumber}</span></span>
                  <span style={styles.roundStat}>Least Frequent: <span style={styles.roundStatValue}>{roundStats.leastFrequentNumber}</span></span>
                </div>
              )}
            </div>
              ...
              <div style={styles.selectedNumberDetail}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    ...
                    <p style={{ margin: '0.5rem 0 0 0', color: '#666' }}>Column: {getColumnForNumber(selectedNumber)} ({CONSTANTS.COLUMN_RANGES[getColumnForNumber(selectedNumber)].join('-')})</p>
                  </div>
                ...
                <div style={{ marginTop: '1rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.5rem' }}>
                  <div style={{ background: '#fff', padding: '0.5rem', borderRadius: '8px', textAlign: 'center' }}><div style={{ fontSize: '0.8rem', color: '#666' }}>Times Called</div><div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#667eea' }}>{roundFrequency.find(f => f.number === selectedNumber)?.count || 0}</div></div>
                  <div style={{ background: '#fff', padding: '0.5rem', borderRadius: '8px', textAlign: 'center' }}><div style={{ fontSize: '0.8rem', color: '#666' }}>Status</div><div style={{ fontSize: '1rem', fontWeight: 'bold' }}>{highlightNumbers.includes(selectedNumber) ? '✅ Called' : '❌ Not Called'}</div></div>
                  <div style={{ background: '#fff', padding: '0.5rem', borderRadius: '8px', textAlign: 'center' }}><div style={{ fontSize: '0.8rem', color: '#666' }}>Prime Number</div><div style={{ fontSize: '1rem', fontWeight: 'bold' }}>{isPrime(selectedNumber) ? '✅ Yes' : '❌ No'}</div></div>
                </div>
              </div>
            )}
            {roundFrequency.length === 0 && (<p style={{ textAlign: "center", color: "#999", padding: "2rem" }}>No numbers drawn yet. Start drawing balls to track frequency!</p>)}
          </div>
        )}
          ...
          <div style={styles.ballsSection}>
            ...
            <form onSubmit={handleManualNumberSubmitSafe} style={{ marginBottom: "1.5rem" }}>
              <div style={{ display: "flex", gap: "0.5rem", maxWidth: "400px", margin: "0 auto" }}>
                <input type="number" value={manualNumberInput} onChange={(e) => setManualNumberInput(e.target.value)} placeholder="Enter number (1-75) and press Enter" min="1" max="75" style={{ flex: 1, padding: "0.75rem", border: "2px solid #667eea", borderRadius: "8px", fontSize: "1rem", outline: "none" }} />
              ...
              <small style={{ color: "#666", display: "block", marginTop: "0.25rem", textAlign: "center" }}>Type a number and press Enter to toggle highlight</small>
            </form>
            <div style={styles.ballsGrid}>
              {Array.from({ length: 75 }, (_, i) => i + 1).map((num) => {
                ...
                return (
                  <div key={num} onClick={() => toggleNumberSafe(num)} style={{ ...styles.ball(active), ...(isInHistory && !active ? { background: "#e8f4fd", border: "2px solid #667eea" } : {}), ...(isFavorite ? styles.favoriteBall : {}), position: "relative", display: "flex", flexDirection: "column", padding: "0.25rem" }}>
                    ...
                    {isFavorite && (<span style={{ position: "absolute", top: "-5px", right: "-5px", fontSize: "0.8rem", fontWeight: "bold" }}>⭐</span>)}
                  </div>
                );
              })}
            </div>
            {Object.values(numberFrequency).some(f => f > 0) && (
              <div style={{ marginTop: "1.5rem", padding: "1rem", background: "linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)", borderRadius: "12px" }}>
                <h4 style={{ margin: "0 0 0.5rem 0" }}>📊 Round Frequency Summary</h4>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.5rem" }}>
                  <div><strong>Numbers called multiple times:</strong> {Object.entries(numberFrequency).filter(([_, count]) => count > 1).map(([num, count]) => `${num} (${count}x)`).join(', ') || 'None'}</div>
                  <div><strong>Numbers called once:</strong> {Object.entries(numberFrequency).filter(([_, count]) => count === 1).map(([num]) => num).join(', ') || 'None'}</div>
                  <div><strong>Most frequent:</strong> {Object.entries(numberFrequency).filter(([_, count]) => count > 0).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([num, count]) => `${num} (${count}x)`).join(', ')}</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ background: "white", borderRadius: "16px", padding: "1.5rem", marginBottom: "2rem", border: "2px solid #667eea", boxShadow: "0 10px 30px rgba(102, 126, 234, 0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "1rem", background: "#f5f5f5", padding: "0.5rem 1rem", borderRadius: "40px" }}>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#4CAF50" }}>🏆</span><span>Winners: {myRankedCards.filter(c => c.isWinner).length}</span></span>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#ffd700" }}>📌</span><span>Pinned: {myRankedCards.filter(c => c.isPinned).length}</span></span>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#667eea" }}>⭐</span><span>With Favorites: {myRankedCards.filter(c => c.favoriteCount > 0).length}</span></span>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
              <div style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", padding: "1rem", borderRadius: "12px", color: "white" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Average Win %</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{Math.round(myRankedCards.reduce((a, b) => a + b.winPercentage, 0) / myRankedCards.length)}%</div></div>
              <div style={{ background: "linear-gradient(135deg, #ffd700 0%, #ffb347 100%)", padding: "1rem", borderRadius: "12px", color: "#333" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Best Card</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{Math.max(...myRankedCards.map(c => c.winPercentage)).toFixed(0)}%</div></div>
              <div style={{ background: "linear-gradient(135deg, #4CAF50 0%, #45a049 100%)", padding: "1rem", borderRadius: "12px", color: "white" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Pattern Winners</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{myRankedCards.filter(c => c.isWinner).length}</div></div>
            </div>
                ...
                <div key={`${section}-${idx}`} className="card-container" style={styles.cardContainer} onClick={() => toggleCardFlip(idx, section)}>
                  <div className="card-actions" style={styles.cardActions}>
                    {editingCard !== `${section}-${idx}` ? (
                      <>
                        <button onClick={(e) => togglePin(idx, section, e)} style={{...styles.actionButton, ...styles.pinButton(isPinned)}} className="action-button" title={isPinned ? "Unpin card" : "Pin card"}>📌</button>
                        <button onClick={(e) => startEdit(card, idx, section, e)} style={{...styles.actionButton, ...styles.editButton}} className="action-button" title="Edit card">✏️</button>
                        <button onClick={(e) => startEditLabel(idx, section, e)} style={{...styles.actionButton, ...styles.labelButton}} className="action-button" title="Edit label">🏷️</button>
                        <button onClick={(e) => saveCard(card, idx, section, e)} style={{...styles.actionButton, ...styles.saveButton}} className="action-button" title="Save to collection">💾</button>
                        <button onClick={(e) => deleteCard(idx, section, e)} style={{...styles.actionButton, ...styles.deleteButton}} className="action-button" title="Delete card">🗑️</button>
                      </>
                    ) : (
                      <>
                        <button onClick={(e) => saveEdit(idx, section, e)} style={{...styles.actionButton, ...styles.saveButton}} className="action-button" title="Save changes">✓</button>
                        <button onClick={(e) => cancelEdit(e)} style={{...styles.actionButton, ...styles.cancelButton}} className="action-button" title="Cancel">✕</button>
                      </>
                    )}
                  </div>
                          ...
                          {[0,1,2,3,4].map((row) => (
                            <div key={row} style={styles.bingoRow}>
                              {[0,1,2,3,4].map((col) => {
                                ...
                                const isFavorite = !isFree && cell !== "★" && favoriteNumbersList.includes(cell);
                                const hasError = hasCellError(col, row);
                                if (editingCard === `${section}-${idx}`) {
                                  return (<input key={col} type="text" value={cell} onChange={(e) => updateCell(col, row, e.target.value, e)} onClick={(e) => e.stopPropagation()} style={styles.editInput(hasError)} disabled={cell === "FREE"} title={hasError ? validationErrors[`cell_${col}_${row}`] || validationErrors[`col_${col}`] : ""} />);
                                }
                                return (
                                  <div key={col} style={styles.bingoCell(isHighlighted, isFree, isFavorite, hasError)} title={hasError ? validationErrors[`cell_${col}_${row}`] || validationErrors[`col_${col}`] : ""}>
                                    {isFree ? "⭐" : cell}
                                    {isFavorite && !isHighlighted && (<span style={styles.favoriteStar}>⭐</span>)}
                                  </div>
                                );
                              })}
                            </div>
        ...
        {activeCardSection === "generated" && generatedCards.length > 0 && (
          <div style={{ background: "white", borderRadius: "16px", padding: "1.5rem", marginBottom: "2rem", border: "2px solid #9C27B0", boxShadow: "0 10px 30px rgba(156, 39, 176, 0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "1rem", background: "#f5f5f5", padding: "0.5rem 1rem", borderRadius: "40px" }}>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#4CAF50" }}>🏆</span><span>Winners: {generatedRankedCards.filter(c => c.isWinner).length}</span></span>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#ffd700" }}>📌</span><span>Pinned: {generatedRankedCards.filter(c => c.isPinned).length}</span></span>
                <span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}><span style={{ color: "#667eea" }}>⭐</span><span>With Favorites: {generatedRankedCards.filter(c => c.favoriteCount > 0).length}</span></span>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
              <div style={{ background: "linear-gradient(135deg, #9C27B0 0%, #BA68C8 100%)", padding: "1rem", borderRadius: "12px", color: "white" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Average Win %</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{Math.round(generatedRankedCards.reduce((a, b) => a + b.winPercentage, 0) / generatedRankedCards.length)}%</div></div>
              <div style={{ background: "linear-gradient(135deg, #ffd700 0%, #ffb347 100%)", padding: "1rem", borderRadius: "12px", color: "#333" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Best Card</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{Math.max(...generatedRankedCards.map(c => c.winPercentage)).toFixed(0)}%</div></div>
              <div style={{ background: "linear-gradient(135deg, #4CAF50 0%, #45a049 100%)", padding: "1rem", borderRadius: "12px", color: "white" }}><div style={{ fontSize: "0.9rem", opacity: 0.9 }}>Pattern Winners</div><div style={{ fontSize: "2rem", fontWeight: "bold" }}>{generatedRankedCards.filter(c => c.isWinner).length}</div></div>
            </div>
            <div style={styles.cardsContainer}>
          ...
          <div style={styles.labelModal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.emojiPicker}>
              {AVAILABLE_EMOJIS.map(emoji => (
                <div key={emoji} style={{ ...styles.emojiOption, background: cardLabels[`${labelCardSection}-${labelCardIndex}`]?.emoji === emoji ? "#667eea" : "transparent", color: cardLabels[`${labelCardSection}-${labelCardIndex}`]?.emoji === emoji ? "white" : "black" }} className="emoji-option"
            ...
            <div style={styles.inputGroup}>
              ...
              <textarea value={cardLabels[`${labelCardSection}-${labelCardIndex}`]?.notes || ""} onChange={(e) => { setCardLabels(prev => ({ ...prev, [`${labelCardSection}-${labelCardIndex}`]: { ...prev[`${labelCardSection}-${labelCardIndex}`], notes: e.target.value } })); }} placeholder="Add notes about this card..." style={styles.textarea} />
            </div>
        ...
        <div style={styles.modal} onClick={() => setShowResults(false)}>
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
              <h2>🏆 Bingo Results</h2>
              <div>{gameResults.length > 0 && (<button onClick={exportResults} style={styles.exportButton}>📥 Export</button>)}</div>
            </div>
            {gameResults.length === 0 ? (
              <p style={{ textAlign: "center", color: "#999", padding: "2rem" }}>No results yet. Click "Record BINGO Result" when you have winners!</p>
            ) : (
              <>
                {stats && (
                    ...
                    <div style={{ marginTop: "1rem" }}>
                      ...
                      <div style={{ display: "grid", gap: "0.5rem" }}>
                        {Object.entries(stats.patternStats).map(([pattern, data]) => (
                          <div key={pattern} style={{ background: "rgba(255,255,255,0.2)", padding: "0.5rem", borderRadius: "8px", display: "flex", justifyContent: "space-between" }}>
                            <span>{data.icon} {data.name}</span><span>{data.count} games, {data.totalWinners} winners</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {result.winners.map(winner => (
                      <div key={winner.cardIndex} style={{ marginTop: "0.5rem", padding: "0.5rem", background: "#f5f5f5", borderRadius: "8px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                          ...
                          <span style={{ fontSize: "0.8rem", color: "#999" }}>({winner.section === "myCards" ? "My Card" : "Generated"})</span>
                        </div>
                      </div>
                    ))}
                    {result.notes && (<div style={{ color: "#666", fontStyle: "italic", marginTop: "0.5rem" }}>📝 {result.notes}</div>)}
                ...
                <div key={savedCard.id} style={styles.savedCardItem}>
                  <div style={styles.savedCardInfo} onClick={() => loadSavedCard(savedCard)}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                      ...
                      <span style={{ fontSize: "0.8rem", color: "#999" }}>({savedCard.section === "myCards" ? "My Card" : "Generated"})</span>
                    </div>
