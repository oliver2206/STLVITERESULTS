...
function Pattern({ goBack }) {
  ...
  const [winChancePreviewPattern, setWinChancePreviewPattern] = useState("blackout");
  ...
  const checkEmptyCross = useCallback((card) => {
    for (let r = 0; r < 5; r++) {
      for (let c = 0; c < 5; c++) {
        ...
        if (isInnerCross) continue; // skip inner cross arms and center
        const num = card[c][r];
  ...
  // Returns a Set of "col,row" keys that are part of the given pattern shape
  const getPatternCells = useCallback((patternId) => {
    ...
    switch(patternId) {
      case "blackout":
        ...
        break;
      case "t":
        for (let c = 0; c < 5; c++) cells.add(`${c},0`); // top row
        for (let r = 0; r < 5; r++) cells.add(`2,${r}`); // middle col
        break;
      case "x":
      ...
      case "emptyCross":
        for (let r = 0; r < 5; r++) for (let c = 0; c < 5; c++) {
          // Exclude inner cross arms (the 4 cells adjacent to center) and center itself
          const isInnerCross = (r === 2 && (c === 1 || c === 2 || c === 3)) || (c === 2 && (r === 1 || r === 2 || r === 3));
  ...
  const generateCardWithFavoriteNumbers = useCallback((useFavorites = true) => {
    ...
    for (let col = 0; col < 5; col++) {
      ...
      const tempFav = colPool.filter(n => favorites.includes(n));
      const tempNonFav = colPool.filter(n => !favorites.includes(n));

      // Separate pattern rows from non-pattern rows (skip FREE cell at col=2, row=2)
      const patternRows = [];
      const nonPatternRows = [];
      for (let row = 0; row < 5; row++) {
        if (col === 2 && row === 2) continue;
        if (patternCells.has(`${col},${row}`)) patternRows.push(row);
        else nonPatternRows.push(row);
      }

      // Assign a number to a row: lucky numbers go to pattern rows first
      const pickFrom = (pool) => {
      ...
      for (const row of patternRows) {
        const num = tempFav.length > 0 ? pickFrom(tempFav) : pickFrom(tempNonFav);
        if (num != null) { card[col][row] = num; usedNumbers.add(num); }
      }
      const remaining = [...tempFav, ...tempNonFav];
      for (const row of nonPatternRows) {
        const num = pickFrom(remaining);
        if (num != null) { card[col][row] = num; usedNumbers.add(num); }
      }
    }
    ...
    card[2][2] = "FREE";
    return card;
  }, [favoriteNumbersList, getPatternCells, currentPattern]);
  ...
  const generateCardWithTargetPercentage = useCallback((targetPercentage, useFavorites = true) => {
    ...
    for (let attempt = 0; attempt < 100; attempt++) {
      ...
      if (Math.abs(percentage - targetPercentage) < Math.abs(bestPercentage - targetPercentage)) { bestCard = card; bestPercentage = percentage; }
        ...
        <div style={styles.controlsGrid}>
          <div style={styles.controlCard}>
            ...
            <button onClick={handleGenerate} disabled={isGenerating} style={{ ...styles.generateButton, opacity: isGenerating ? 0.6 : 1, cursor: isGenerating ? "not-allowed" : "pointer" }}>
              {isGenerating ? "Generating..." : `🎲 Generate Cards for ${PATTERNS.find(p => p.id === winChancePreviewPattern)?.icon || ""} ${PATTERNS.find(p => p.id === winChancePreviewPattern)?.label || currentPattern}`}
            </button>
          </div>
                          ...
                          {[0,1,2,3,4].map((row) => (
                            <div key={row} style={styles.bingoRow}>
                              {[0,1,2,3,4].map((col) => {
                                ...
                                const isLuckyCell = isPatternCell && !isHighlighted && !isFree && currentPattern !== "blackout" && favoriteNumbersList.includes(cell);
                                return (
                                  <div key={col} style={styles.bingoCell(isHighlighted, isFree, isFavorite, hasError, isPatternCell)}>
                                    {isFree ? "⭐" : cell}
                                    {(isFavorite || isLuckyCell) && !isFree && (<span style={styles.favoriteStar}>⭐</span>)}
                                  </div>
                                );
                              })}
                            </div>
